...
document.getElementById('today-date').textContent = new Date().toLocaleDateString('en-US', {
    ...
    (function() {
      ...
      headers.forEach((header, index) => {
        ...
        let pinGate = null;
        if (isPinLocked) {
          header.classList.add('accordion-toggle-locked');
          const lockIcon = document.createElement('span');
          lockIcon.className = 'accordion-lock-icon';
          lockIcon.textContent = '🔒';
          header.insertBefore(lockIcon, caret);

          pinGate = document.createElement('div');
          pinGate.className = 'pin-gate';
          pinGate.hidden = true;
          pinGate.dataset.accordionItem = panelId;
          pinGate.innerHTML =
            '<p class="pin-gate-label">Enter the 4-digit PIN to open this section</p>' +
            '<div class="pin-gate-row">' +
              '<input type="password" inputmode="numeric" pattern="[0-9]*" maxlength="4" class="pin-gate-input" aria-label="4-digit PIN">' +
              '<button type="button" class="pin-gate-submit">Unlock</button>' +
            '</div>' +
            '<p class="pin-gate-error" hidden>Incorrect PIN — try again.</p>';
          header.insertAdjacentElement('afterend', pinGate);
        }

        function openPanel() {
          header.setAttribute('aria-expanded', 'true');
          caret.textContent = '▲';
          panelItems.forEach((item) => { item.hidden = false; });
        }
        function closePanel() {
          header.setAttribute('aria-expanded', 'false');
          caret.textContent = '▼';
          panelItems.forEach((item) => { item.hidden = true; });
        }
        function togglePanel() {
          const isOpen = header.getAttribute('aria-expanded') === 'true';
          if (isOpen) { closePanel(); return; }
          if (isPinLocked) {
            const input = pinGate.querySelector('.pin-gate-input');
            pinGate.hidden = false;
            pinGate.querySelector('.pin-gate-error').hidden = true;
            input.value = '';
            input.focus();
            return;
          }
        ...
        if (isPinLocked) {
          const input = pinGate.querySelector('.pin-gate-input');
          const submitBtn = pinGate.querySelector('.pin-gate-submit');
          const errorMsg = pinGate.querySelector('.pin-gate-error');
